  ...
  if (V->variables) {
	...
	for (l = 1; l < V->variables; l++) {
	  ...
	  for (k = 0; k < R->np; k++) {
	    ...
	    OutputData(nt, nr, O, Oo, R->Y[k], R->value[k]);
	    if (!isnan(O[l])) {
	      if (firsttime) {
		fprintf(fp, "%d\n", l);
		firsttime = FALSE;
	      }
	      fprintf(fp, "%f %.3e %.3e\n", R->Y[k][0]*V->scaletime, O[l],
		      Oo[l]);
	    }
	  }
	}
			 ...
			 int nt, int nr, int *status)
{
  ...
  if ((*status = Model(nt, nr, n, theta, y, &T)) == SUCCESS &&
      (Time_Index(Ind, n, y, t))) {
    logL = 0;
    for (i = 0; i < Ind->np; i++)
      logL += logLikelihood(nt, nr, theta, y[t[i]], Ind->Y[i], Ind->value[i]);
      /* logL += logLikelihoodI(nt, nr, theta, y, Ind->Y[i], Ind->value[i], n); */
  }
  else
    logL = -INFINITY;

  free(t); free(y[0]); free(y); free(T.t);
  return logL;
}
