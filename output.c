...
void PostAnalysis(struct simulation *Sim, struct data *Data)
{
  ...
  np1 = function(nt1, nr1, NULL, NULL, NULL, &TP);
  if (np1 < 1 || np1 > 1e8)
    return;
  else if (Sim->verbose)
  ...
  for (nt = nt1; nt <= NT2; nt++) if (Sim->trtused[nt])
    ...
    TP.t[k] = Ind->Y[k][0];
      ...
      count = 0;
      for (l = 0; l < ns; l++) {
    function(nt, nr, x1, y1, samples[nt][nr][l], &TP);
    for (k = 0; k < np1; k++)
      d1[k][count] = y1[k];
    count++;
      }
          ...
          double ***fitdist, gsl_rng *stream)
{
  ...
  for (k = 0; k < Ind->np; k++) {
    ...
    OutputData(nt, nr, O, Oo, Y, truevector);
    for (l = 1; l < V->variables; l++)
      simdist[l][k][i] = O[l];
  }
...
#     pragma omp parallel private(i) num_threads(Sim->threads)
      ...
      for (l = 1; l < V->variables; l++) {
        ...
        for (k = 0; k < Ind->np; k++) {
          ...
          OutputData(nt, nr, O, Oo, Ind->Y[k], Ind->value[k]);
          if (!isnan(O[l])) {
            if (firsttime) {
              fprintf(fps, "%d\n", l);
              firsttime = FALSE;
...
void SimulatedData(struct simulation *Sim, struct data *Data)
{
  ...
  for (nt = nt1; nt <= NT2; nt++) if (Sim->trtused[nt]) {
    ...
    SimulateData(nt, nr, Ysim, y[t_index[k]], theta, Ind->Y[k], 
      ...
      free(theta); free(y[0]); free(y); free(t_index); free(Ysim); free(TP.t);
    }
  }
  gsl_rng_free(stream);
}
